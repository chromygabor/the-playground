...
import com.chromy.frpui.fw.core.{Context, SideEffect, Event, SideEffectChain}
 ...
 */
object JavaFX {
  def apply[M <: BaseController : Manifest](context: Context, render: SideEffectChain[M#C], initialState: M#C): Try[(Parent, M, SideEffect)] = {
    val ct = manifest[M]
    ...
    Try {
      ...
      val sideEffect = controller.init(context, render.asInstanceOf[SideEffectChain[controller.C]], initialState.asInstanceOf[controller.C])
